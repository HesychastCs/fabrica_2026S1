...
class SavingGoalServiceTest {
    ...
    @Mock private SavingGoalRepositoryPort savingGoalRepositoryPort;
    ...
    class RegistrarAporte {
        ...
        void ca01_ca02_aporteAumentaAcumuladoYPorcentaje() {
            ...
            when(savingGoalRepositoryPort.findById(goalId)).thenReturn(Optional.of(metaEnProgreso));
            when(savingGoalRepositoryPort.updateAvance(eq(goalId), eq(nuevoAvance), eq(GoalStatus.EN_PROGRESO)))
                    .thenReturn(metaActualizada);

            // Simulamos la lógica de aporte directamente (el servicio delega updateAvance)
            SavingGoal result = savingGoalRepositoryPort.updateAvance(goalId, nuevoAvance, GoalStatus.EN_PROGRESO);
        ...
        void ca03_metaMarcadaCompletadaAlAlcanzarObjetivo() {
            ...
            when(savingGoalRepositoryPort.findById(goalId)).thenReturn(Optional.of(metaCasiCompleta));
            when(savingGoalRepositoryPort.updateAvance(eq(goalId), eq(5_000_000), eq(GoalStatus.COMPLETADA)))
                    .thenReturn(metaCompletada);

            SavingGoal result = savingGoalRepositoryPort.updateAvance(goalId, 5_000_000, GoalStatus.COMPLETADA);
        ...
        void ca04_metaCompletadaNoPermiteNuevosAportes() {
            ...
            when(savingGoalRepositoryPort.findById(goalId)).thenReturn(Optional.of(metaCompletada));
            ...
            assertThatThrownBy(() ->
                    savingGoalRepositoryPort.updateAvance(goalId, 100_000, GoalStatus.COMPLETADA))
