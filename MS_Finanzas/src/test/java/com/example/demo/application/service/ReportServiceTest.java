...
class ReportServiceTest {
    ...
    @Mock private ReportRepositoryPort reportRepositoryPort;
    ...
    class VerReporteMensual {
        ...
        void ca01_ca02_reporteConDatosMuestraTotalesCorrectos() {
            ...
            when(transactionRepositoryPort.sumByTitularAndTypeAndMonth(titularId, TypeTransaction.INGRESO, ANHO, MES))
                    .thenReturn(ingresos);
            when(transactionRepositoryPort.sumByTitularAndTypeAndMonth(titularId, TypeTransaction.GASTO, ANHO, MES))
                    .thenReturn(gastos);
            when(transactionRepositoryPort.sumByTitularAndTypeAndMonth(titularId, TypeTransaction.APORTE_META, ANHO, MES))
                    .thenReturn(aportes);
            when(transactionRepositoryPort.sumByTitularAndTypeAndMonth(titularId, TypeTransaction.RETIRO_META, ANHO, MES))
                    .thenReturn(retiros);

            Report reportSaved = new Report(UUID.randomUUID(), MES, ANHO,
                    ingresos, gastos, aportes.subtract(retiros), balanceEsperado, Instant.now(), titular);
            ...
            assertThat(result.ingresos()).isEqualByComparingTo(ingresos);
            assertThat(result.gastos()).isEqualByComparingTo(gastos);
        ...
        void ca03_sinMovimientosLanzaExcepcion() {
            ...
            when(titularRepositoryPort.findById(titularId)).thenReturn(Optional.of(titular));
            when(transactionRepositoryPort.sumByTitularAndTypeAndMonth(titularId, TypeTransaction.INGRESO, anhoVacio, mesVacio))
                    .thenReturn(null);
            when(transactionRepositoryPort.sumByTitularAndTypeAndMonth(titularId, TypeTransaction.GASTO, anhoVacio, mesVacio))
                    .thenReturn(null);
            when(transactionRepositoryPort.sumByTitularAndTypeAndMonth(titularId, TypeTransaction.APORTE_META, anhoVacio, mesVacio))
                    .thenReturn(null);
            when(transactionRepositoryPort.sumByTitularAndTypeAndMonth(titularId, TypeTransaction.RETIRO_META, anhoVacio, mesVacio))
                    .thenReturn(null);
